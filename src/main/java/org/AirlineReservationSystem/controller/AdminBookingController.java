...
public class AdminBookingController {
	...
	@PostMapping("/save")
	public String saveBooking(HttpServletRequest req, @RequestParam(required = false) Long id, @RequestParam Long userId, @RequestParam Long flightId, @RequestParam int seats, @RequestParam SeatClass seatClass, @RequestParam(required = false) BookingStatus status, Model model, RedirectAttributes ra) {
		if (isNotAdmin(req)) return "redirect:/login";

		try {
			boolean isNewBooking = id == null;
			Booking booking = isNewBooking ? bookingService.createBooking(userId, flightId, seatClass, seats) : bookingService.updateBooking(id, userId, flightId, seatClass, seats, status);
			String flightNumber = booking.getFlight() != null ? booking.getFlight().getFlightNumber() : "N/A";
			ra.addFlashAttribute("successMessage", (isNewBooking ? "Booking created" : "Booking updated") + ": #" + booking.getId() + " for flight " + flightNumber + ".");
