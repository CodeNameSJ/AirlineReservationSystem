...
public class FlightService {
  ...
  public List<Flight> search(
    ...
    if (!hasOrigin && !hasDestination) {
      return hasDateRange
          ? flightRepo.findByDepartureTimeBetween(start, end)
          : flightRepo.findAll();
    }

    if (hasOrigin && hasDestination) {
      return hasDateRange
          ? flightRepo.findByOriginAndDestinationAndDepartureTimeBetween(
    ...
    if (hasOrigin) {
      return hasDateRange
          ? flightRepo.findByOriginAndDepartureTimeBetween(origin, start, end)
          : flightRepo.findByOrigin(origin);
    }

    return hasDateRange
        ? flightRepo.findByDestinationAndDepartureTimeBetween(destination, start, end)
        ...
        .ifPresent(
            existing -> {
              throw new IllegalArgumentException(
                  ErrorConstants.FLIGHT_NUMBER_EXISTS_ERROR.getMessage());
            });
  ...
  public int reserveSeatsAtomic(Long flightId, SeatClass seatClass, int seats) {
    ...
    int updated =
        (seatClass == SeatClass.ECONOMY)
            ? flightRepo.reserveEconomySeats(flightId, seats)
  ...
  public int releaseSeatsAtomic(Long flightId, SeatClass seatClass, int seats) {
    ...
    if (updated == 0) {
      ...
      throw new IllegalStateException(
          ErrorConstants.SEAT_RELEASE_EXCEEDS_CAPACITY_ERROR.getMessage());
    }
