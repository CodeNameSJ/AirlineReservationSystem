...
class BookingSystem implements Bookable {
	...
	public Flight findFlight(String flightNo) {
		for (int i = 0; i < flightCount; i++) if (flights[i].getFlightNo().equalsIgnoreCase(flightNo)) return flights[i];
		return null;
	}
	...
	public void book(Passenger p) {
		...
		if (!f.hasSeatAvailable(seat)) {
			System.out.println("🚫 No " + seat + " seats available!");
			return;
		}
		double price = f.getPrice(seat);
		...
		f.reserveSeat(seat);
		Ticket t = (seat == SeatClass.ECONOMY)
				           ? new EconomyTicket(p, f)
				           : new BusinessTicket(p, f);
		tickets[ticketCount++] = t;
		System.out.println("╚════════════════════════════════╝\n");
		System.out.println("✅ Ticket Booked Successfully!");
		System.out.println("🆔 Ticket ID: " + t.getTicketId() + " | Passenger: " + p + " | Seat: " + seat);
	}
