...
class UserControllerTest {
  ...
  void setUp() {
    mockMvc =
        MockMvcBuilders.standaloneSetup(
                new UserController(flightService, bookingService, pricingService))
            .build();
  }
  ...
  void redirectsWhenBookingDoesNotBelongToCurrentUser() throws Exception {
    ...
    mockMvc
        .perform(get("/user/booking/99").sessionAttr("userId", 8L))
  ...
  void loadsBillForOwnedBookingWithPricingBreakdown() throws Exception {
    ...
    mockMvc
        .perform(get("/user/booking/99/bill").sessionAttr("userId", 7L))
  ...
  void cancelAddsSuccessFlashMessage() throws Exception {
    mockMvc
        .perform(post("/user/bookings/cancel").sessionAttr("userId", 7L).param("id", "99"))
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectedUrl("/user/home"))
